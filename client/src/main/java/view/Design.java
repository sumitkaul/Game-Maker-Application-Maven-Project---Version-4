...
public class Design implements Resizable, ActionListener {
    ...
    /**
     * @param shouldDisplayScore
     *            the shouldDisplayScore to set
     */
	...
	/*
	 * Menu menuPlayProgress = new Menu("Play Progress");
	 * menuBar.add(menuPlayProgress);
	 * 
	 * // Create a menu item MenuItem itemPlayProgress1 = new
	 * MenuItem("Load"); MenuItem itemPlayProgress2 = new MenuItem("Save");
	 * 
	 * itemPlayProgress1.addActionListener(new ActionListener() {
	 * 
	 * @Override public void actionPerformed(ActionEvent e) { // TODO
	 * Auto-generated method stub GameProgressLoadPanel p = new
	 * GameProgressLoadPanel(controlPanel);
	 * 
	 * String gameData = p.readGameDataFromRemoteList();
	 * 
	 ...
	 * ClockDisplay.getInstance().setVisible(game.isClockDisplayable()); //
	 * SpriteList.getInstance().setSpriteList(allSpriteModels);
	 * SpriteList.getInstance
	 * ().setSelectedSpriteModel(allSpriteModels.get(0));
	 * layerBox.removeAllItems(); for (String layer : layers) {
	 * layerBox.addItem(layer); }
	 * 
	 * facade.getGameController().setEvents(game.getEventsForGameController()
	 * ); facade.getKeyListenerController().setKeyEvents(game.
	 * getEventsForKeyController());
	 * 
	 * facade.createViewsForModels(game.getSpriteList());
	 * 
	 * for (SpriteModel model : allSpriteModels) {
	 ...
	 * actionEventPanel.getSpriteListIndividualModel().addElement(model.getId
	 * ()); if
	 * (!actionEventPanel.getSpriteListGroupModel().contains(model.getGroupId
	 * ())) {
	 * actionEventPanel.getSpriteListGroupModel().addElement(model.getGroupId
	 * ()); } if (actionEventPanel.getSpriteListIndividualModel().size() >
	 * 0) { actionEventPanel.getSpriteList().setModel(actionEventPanel.
	 * getSpriteListIndividualModel()); } }
	 * 
	 * updateProperties(); } });
	 * 
	 * itemPlayProgress2.addActionListener(new ActionListener() {
	 * 
	 * @Override public void actionPerformed(ActionEvent e) { GamePackage
	 * game = new GamePackage(SpriteList.getInstance().getSpriteList(),
	 * facade.getGameController().getEvents(),
	 * facade.getKeyListenerController().getKeyEvents(),
	 * Layers.getInstance().getLayers(),
	 * ClockDisplay.getInstance().isVisible()); String gameData =
	 * GameDataPackageIO.convertGamePackageToString(game);
	 * GameProgressSavePanel p = new GameProgressSavePanel(controlPanel);
	 * 
	 * p.saveGameToRemoteServer(gameData);
	 * 
	 * } });
	 * 
	 * menuPlayProgress.add(itemPlayProgress1);
	 * menuPlayProgress.add(itemPlayProgress2);
	 * 
	 * MenuItem itemTopScores = new MenuItem("See Top Scores");
	 * menuPlayProgress.addActionListener(new ActionListener() {
	 * 
	 * @Override public void actionPerformed(ActionEvent ae) {
	 * TopScoresPanel p = new TopScoresPanel(controlPanel);
	 * p.readGameScoresFromRemoteList(); } });
	 * menuPlayProgress.add(itemTopScores);
	 * 
	 * this.baseFrame.setMenuBar(menuBar);
	 */
...
=======
	ImageActionListener listener = this.new ImageActionListener();
	for (int i = 0; i < list.size(); i++) {
>>>>>>> branch 'master' of ssh://softeng@tintin.cs.indiana.edu/~/repos/fall2012/a9/team3
