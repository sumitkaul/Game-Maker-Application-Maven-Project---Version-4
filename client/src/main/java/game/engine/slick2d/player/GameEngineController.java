...
public class GameEngineController extends BasicGame {
	...
	private static final Logger LOG = Logger
			.getLogger(GameEngineController.class.getName());
	...
	private GamePackage loadGameData(int loadMode, String[] paras) {
		...
		if (loadMode == LOAD_MODE_LOCAL) {
			gamePackage = GameDataPackageIO
					.loadGamePackageFromLocalFile(paras[0]);
			...
			String gameData = ClientHandler.loadGameBase(paras[0], paras[1],
					paras[2], new Exception[1]);
			gamePackage = GameDataPackageIO.loadGamePackageFromFile(gameData);
		}
	...
	public void init(GameContainer gc) throws SlickException {
		try {
			...
			java.util.logging.Logger.getLogger(
					GameEngineController.class.getName()).log(Level.SEVERE,
					null, ex);
		}
		initActionEvents();
	...
	public Image getImageFromBytes(byte[] imageData, String imageName) {
		Image image = null;
		try {
			image = new Image(new ByteArrayInputStream(imageData), imageName,
					false);
		} catch (Exception ex) {
			LOG.error(ex);
			return null;
	...
	public void initActionEvents() {
		...
		for (EventListener keyevent : keyEvents) {
			...
			String gid = "Bomb";
			Image entityImage = imagesOfSprites
					.get(key.getRegisteredObjectId());
	...
	public void imageDraw(SpriteModel sprite) {
		imagesOfSprites.get(sprite.getId()).setRotation(
				PhysicsComponent.bodies.get(sprite.getId()).getAngle());
		imagesOfSprites.get(sprite.getId()).draw((float) sprite.getPosX(),
				(float) sprite.getPosY(), (float) sprite.getWidth(),
				(float) sprite.getHeight());// (float) bodyPostion.x * 30,
											// (float) bodyPostion.y * 30,
	...
	public void initSpriteImageMapping() throws IOException {
		...
		for (SpriteModel sprite : allSpriteModels) {
			...
			Resources r = ClientHandler.loadResource(rid, Constants.HOST,
	...
	public void initSpriteBodyMapping(SpriteModel sprite) {
		try {
			PhysicsComponent.bodies.put(sprite.getId().toString(),
					physicsComponent.createBody(sprite.getId(), "polygon",
							"Dynamic", (float) sprite.getPosX(),
							(float) sprite.getPreviousY(),
							(float) sprite.getWidth(),
	...
	public void updateKeyEventBinding(GameContainer gc) {
		...
		for (Integer keycode : keyReg.keySet()) {
			try {
				...
				LOG.error("don't worry, it is only temp key-mapping error, we not done yet: "
						+ e);
			}
		}
